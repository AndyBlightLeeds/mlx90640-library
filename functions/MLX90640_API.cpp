...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    while(dataReady == 0)
    {
        ...
        if(error != 0)
        {
            printf("statusRegister read error 1\n");
            return error;
        }
    ...
    while(dataReady != 0 && cnt < 5)
    {
        ...
        if(error == -1)
        {
            printf("statusRegister write error \n");
            return error;
        }

        // Read frame, 832 bytes.
        ...
        if(error != 0)
        {
            printf("statusRegister read error 2\n");
            return error;
        }
        dataReady = statusRegister & 0x0008;
        cnt = cnt + 1;
    ...
    if(error != 0)
    {
        printf("controlRegister read error\n");
        return error;
    } else {
        frameData[832] = controlRegister1;
